...
{% macro grant_semantic_views_privileges(exclude_schemas, grant_roles) %}
    ...
    {% do log('grant_semantic_views_privileges: processing ' ~ (include_schemas | length) ~ ' schemas for roles ' ~ (snowflake_roles | join(', ')), info=True) %}
    {% for schema_name in include_schemas %}
        {% do dbt_monitorial_snowflake_cortex._grant_semantic_views_privileges_specific(schema_name, snowflake_roles) %}
    {% endfor %}
{% endmacro %}